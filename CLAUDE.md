...
- **iwara** (`iwara.tv/video/`): `util.Iwara.resolve_mp4()`. No nicovrc. Re-resolve near playback; the view URL expires in about an hour.
...
- **iwara does not use nicovrc.** `GET https://api.iwara.tv/video/{id}` returns `fileUrl`. `X-Version` is `sha1("{file-uuid}_{expires}_mSvL05GfEmeEmsEYfGCnVpEjYgTJraJN")`. That list includes `Source`; without the header only `360` and `preview` come back. Prefer `src.view` (protocol-relative, host changes). No login or cookie. `fileUrl` expires about an hour after the API call, so resolve at request time. Private or missing videos are 502.
